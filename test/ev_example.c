...
int
set_linger(int fd, int onoff, int linger) {
...
read_cb(struct bufferevent *bev, void *ctx) {
	struct client_ctx_st* client_ctx = ctx;
		...
		void *ctx)
{
	...
	struct bufferevent* bev = bufferevent_socket_new(base, nfd, BEV_OPT_CLOSE_ON_FREE);
	assert(bev);

	struct client_ctx_st* client_ctx = malloc(sizeof(*ctx));
...
main(void) {
	...
	int ret = evutil_parse_sockaddr_port("0.0.0.0:9876", &saddr, &socklen);
	assert(ret == 0);
